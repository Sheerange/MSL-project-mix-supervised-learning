...
# from lql_method import Msl
from faker_method import Msl
import copy
...
def main():
    ...
    # The switch of MSL method. If you want use MSL to train your model, turn it into True
    msl_switch = True if args.msl != 0 else False
    ...
    print("msl_switch: ", msl_switch)
    print('*' * 25)
    print('Epoch\tTrainLoss\tTrainAcc\tTestAcc')
    print('-' * 50)

    if dataset == 'cifar100':
        data_root = os.path.abspath(os.path.join(os.getcwd(), '../datasets/cifar100'))  # get data root path
    if dataset == 'tiny_ImageNet':
        data_root = os.path.abspath(os.path.join(os.getcwd(), '../datasets/tiny-imagenet-200'))  # get data root path
    if dataset == 'CUB200':
        data_root = os.path.abspath(os.path.join(os.getcwd(), '../datasets/CUB-200(+test)'))  # get data root path
    ...
    for run in range(1):
        ...
        if msl_switch:
            ...
            Msl_mode = Msl(net=net, pseudo_loader=val_loader, train_dataset = train_dataset, confidence=confidence, lambda_aug=args.lambda_aug)
        else:
            print("Not use MSL method in your training")
...
# 保留当前模型参数，回溯点
            ...
            if msl_switch and ((best_acc_val > threshold_acc) or epoch>args.epoch_start):  # 判断模型是否收敛？
                print(
                    'before MSL: \tval_acc: {:.2f}%\ttest_acc: {:.2f}\tval_acc_aug: {:.2f}\ttest_acc_aug: {:.2f}\tval_acc5: {:.2f}\ttest_acc5: {:.2f}'.format(val_acc_epoch, test_accuracy, val_acc_epoch_aug, test_accuracy_aug, val_acc5, test_acc5))
                current_lr = optimizer.param_groups[0]['lr']
                print('the current lr is:{}'.format(current_lr))
                print('using MSL!!!' + '***' * 10)
                net, pseudo_output, trully_output = Msl_mode(net, batch_size, optimizer=optimizer, nw=nw)
            ...
            msl_val, msl_val_aug, val_acc5 = _accuracy_(net, validate_loader)
            msl_test, msl_test_aug, test_acc5= _accuracy_(net, test_loader)
            ...
            print('The accuracy on test set is: {}'.format(msl_test))

            is_best = msl_val >= best_acc_val
            if is_best:
            ...
            print(
                'epoch: {}\ttrain_loss: {:.4f}\ttrain_acc: {:.2f}%\ttrain_aug_acc: {:.2f}%\tval_acc: {:.2f}%\ttest_acc: {:.2f}\tval_acc_aug: {:.2f}%\ttest_acc_aug: {:.2f}\tval_acc5: {:.2f}\ttest_acc5: {:.2f}'.format(
                    ...
                    train_acc_epoch, train_acc_epoch_aug, msl_val, msl_test, msl_val_aug, msl_test_aug, val_acc5, test_acc5))
            best_acc_train = max(train_acc_epoch, best_acc_train)
            best_acc_val = max(msl_val, best_acc_val)
            if best_acc_val == msl_val:
                best_acc_test = msl_test
                best_acc5_val = val_acc5
                best_acc5_test = test_acc5

#####   rewind
            if msl_val < val_acc_epoch - args.rewind_threshold:
